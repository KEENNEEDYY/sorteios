...
from lista import *
from jogos import *
import script_pandas_V1

jogos = []
toplist1 = list(toplist)
listaTodosJogos1 = list(listaTodosJogos)
listaTodosJogos2 = script_pandas_V1.return_jogos_in_list()
...
def aposta():
    ...
    while(len(numbers) < 6):
        ...
        random.seed(seed)
        number = random.choice(dado_viciado)
        if number not in numbers:
            numbers.append(number)
        if len(numbers) == 6:
            numbers.sort()
            if tuple(numbers) not in listaTodosJogos2:
                jogos.append((numbers))
                listaTodosJogos2.append(tuple(numbers))
